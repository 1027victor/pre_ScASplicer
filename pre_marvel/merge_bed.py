...
@ray.remote
def process_counts_file(bed_ref, counts_file_path, counts_file):
    """
    Function to process a single counts file using Ray's remote function for parallelization.
    """
    # Get BED data from Ray object store
    bed = ray.get(bed_ref)
    ...
    for i in range(len(bed)):
        ...
        coords = bed_small['chr'] + ':' + positions.astype(str)
        # Calculate total counts
        counts_total = df.loc[df['coord.intron'].isin(coords), 'count'].sum()
        # Save result
        coord_intron = f"{bed_small['chr']}:{bed_small['upstreamEE'] + 1}:{bed_small['downstreamES']}"
...
def process_and_combine_counts(bed_file, counts_dir, output_dir, combined_output, cpus):
    ...
    # Put BED data into Ray object store
    bed_ref = ray.put(bed)
    ...
    for counts_file in counts_files:
        ...
        future = process_counts_file.remote(bed_ref, counts_file_path, counts_file)
